...
__INIT void ioapic_all_init() {
    ...
    out8(ICW1_INIT + ICW1_ICW4, PIC1_CMD);
    out8(ICW1_INIT + ICW1_ICW4, PIC2_CMD);
    out8(0x20, PIC1_DAT);       // ICW2: map master PIC vector base
    out8(0x28, PIC2_DAT);       // ICW2: map slave PIC vector base
    out8(4, PIC1_DAT);          // ICW3: slave PIC at IRQ2 (0000 0100)
    out8(2, PIC2_DAT);          // ICW3: slave PIC's identity (0000 0010)
    out8(ICW4_8086, PIC1_DAT);
    out8(ICW4_8086, PIC2_DAT);
    out8(0xff, PIC1_DAT);       // mask all pins on master chip
    out8(0xff, PIC2_DAT);       // mask all pins on slave chip
